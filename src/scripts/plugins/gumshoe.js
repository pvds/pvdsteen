(function (root, factory) {
  ...
  const getNavs = function () {
    ...
    const navLinks = document.querySelectorAll(settings.selector);

    // For each link, create an object of attributes and push to an array
    forEach(navLinks, function (nav) {
      if (!nav.hash) return;
      const target = document.querySelector(nav.hash);
      if (!target) return;
      navs.push({
        nav: nav,
        target: target,
   ...
   * @private
   */
  const deactivateCurrentNav = function () {
    if (currentNav) {
   ...
   */
  const activateNav = function (nav) {
  ...
  gumshoe.getCurrentNav = function () {
    ...
    const position = root.pageYOffset;
   ...
   * @private
   */
  const setInitCurrentNav = function () {
    forEach(navs, function (nav) {
   ...
   */
  const scrollStop = function (event) {
